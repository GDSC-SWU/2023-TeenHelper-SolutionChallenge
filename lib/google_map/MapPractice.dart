...
class MapPracticeState extends State<MapPractice> {
  ...
  Future<BitmapDescriptor> _loadCustomMarkerImage() async {
    final ImageConfiguration config = ImageConfiguration(
        size: Size(ScreenUtil().setWidth(32.0), ScreenUtil().setHeight(32.0)));
    final bitmap = await BitmapDescriptor.fromAssetImage(
        config, 'images/map_icon_mark.png');
    return bitmap;
  }

  void addCustomIcon() {
    BitmapDescriptor.fromAssetImage(
            const ImageConfiguration(), "images/map_icon_mark.png")
        .then((icon) {
      setState(() {
        markerIcon = icon;
      });
  ...
  Future<void> searchingname() async {
    //검색창에 입력한 값 db에서 검색
    if (searchingword != null && searchingword.isNotEmpty) {
      var db = Mysql();
      String sql =
          "select shelter_name, shelter_location, shelter_id from shelter where shelter_location like '%${searchingword}%';";
      var conn = await db.getConnection();
  ...
  Future<void> searchingtag() async {
    //태그 정보를 db에서 조회 리스트 저장
    if (id != null) {
      var db = Mysql();
      String sql =
          "select shelter_name, shelter_location, shelter_id from hospital where hospital_id=${id};";
      var conn = await db.getConnection();
      final results = await conn.query(sql);
  ...
  Future<void> Showreview() async {
    if (_names.isNotEmpty && _names != null) {
      ...
      for (int i = 0; i < _names.length; i++) {
        String sql =
            "select hospital_subject, shelter_name, review_cnt from hospital where shelter_name='${_names[i]}';";
        final results = await conn.query(sql);
        final numbersList =
            results.map((row) => row[2].toString() as String).toList();
        final hospitalsList =
            results.map((row) => row[0].toString() as String).toList();
                ...
                decoration: InputDecoration(
                  prefixIcon: _isFocused
                      ? IconButton(
                          ...
                          icon: Image.asset("images/back_Key.png",
                              width: ScreenUtil().setWidth(24.0),
                              height: ScreenUtil().setHeight(24.0)))
                      : IconButton(
                          onPressed: () {},
                          icon: Image.asset("images/map_icon_address.png",
                  ...
                  suffixIcon: IconButton(
                    ...
                    icon: Image.asset('images/map_icon_search.png',
              ...
              itemBuilder: (context, suggestion) {
                return ListTile(
                  ...
                  leading: Padding(
                    ...
                    child: Image.asset('images/map_search_local.png',
                        width: ScreenUtil().setWidth(16.0),
                        height: ScreenUtil().setHeight(16.0)),
                  ),
                  title: Text(
          ...
          (index) => Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {},
                  child:
                      Text("${review_hospital[index]} 연계 ${review_num[index]}"),
                ),
              )));
      return List<Widget>.generate(
        (buttons.length / 3).ceil(),
        (index) => Row(
  ...
  Stream<List<ScrapModel>> streamScrap(String shelter_name) {
    try {
      ...
      final Stream<QuerySnapshot> snapshots = db
          ...
          .where("shelter_name", isEqualTo: shelter_name)
          .snapshots();
      return snapshots.map((querySnapshot) {
        ...
        querySnapshot.docs.forEach((element) {
          ...
          scrap.add(
              ScrapModel.fromMap(map: element.data() as Map<String, dynamic>));
        });
  ...
  Future<void> scrapEdit(bool scrap, int shelter_id) async {
    await FirebaseFirestore.instance
        .collection("scrap")
        .doc(FirebaseAuth.instance.currentUser!.uid + shelter_id.toString())
        .update({
      "scrap": scrap,
    });
  }

  Future<void> scrapWrite(bool scrap, String shelter_name, int shelter_id,
      String shelter_location) async {
    await FirebaseFirestore.instance
        .collection("scrap")
        .doc(FirebaseAuth.instance.currentUser!.uid + shelter_id.toString())
        .set({
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      children: [
        ...
        //검색창 기능들 구현
        Positioned(
            top: ScreenUtil().setHeight(56.0),
            left: ScreenUtil().setWidth(16.0),
            child: searchingbar()),

        //태그 버튼들
        Positioned(
          top: ScreenUtil().setHeight(116.0),
        ...
        Container(
          //내위치 가져오는 floating button
          child: Positioned(
            bottom: ScreenUtil().setHeight(88),
            right: ScreenUtil().setHeight(16), //네비게이션바 크기만큼 bottom으로 지정할것
            child: Container(
              decoration: BoxDecoration(boxShadow: [
                BoxShadow(
        ...
        Visibility(
          visible:
              (searchingword.isNotEmpty && _suggestions != null) || (id != 0),
                        ...
                        trailing: StreamBuilder<List<ScrapModel>>(
                            ...
                            builder: (context, asyncSnapshot) {
                              if (asyncSnapshot.hasData &&
                                ...
                                return InkWell(
                                    child: Image.asset(scrap[0].scrap == true
                                        ? 'images/staron_btn.png'
                                        : 'images/staroff_btn.png'),
                                    onTap: () => {
                                          scrapEdit(!scrap[0].scrap,
                                              _shelterId[index])
                                        });
                                ...
                                return InkWell(
                                  ...
                                  onTap: () => {
                                    scrapWrite(true, _names[index],
                                        _shelterId[index], _locations[index]),
                                  },
                                );
                              }
                            }),
